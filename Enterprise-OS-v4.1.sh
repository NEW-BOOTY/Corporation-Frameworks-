...
#
# Enterprise-OS-v4.1.sh
# Version: 4.1.0
#
# **FIXED FOR macOS + PRIVILEGED EXECUTION**
#
# **Root Cause Fixes**:
#   1. **Homebrew as root**: Now runs `brew` via `sudo -u $(whoami)` to avoid root execution.
#   2. **`${p^^}` syntax**: Removed (Bash 4+ only). Replaced with `tr '[:lower:]' '[:upper:]'`.
#   3. **Privilege escalation**: Detects if running as root → drops to user for non-privileged ops.
#   4. **Interactive prompts**: Only run when stdin is a TTY.
#
...
#
# **RUN AS SUDO** → Handles privilege drop intelligently.
...
install_pkg() {
  ...
  case "${PKG_MGR}" in
    brew)
      # Homebrew refuses root → run as original user
      if [[ "$(id -u)" -eq 0 ]]; then
...
call_llm() {
  ...
  response=$(curl -s -X POST "${url}" "${headers[@]/#/" -H "}" -d "${payload}" 2>/dev/null || echo "")
...
dispatch_llm() {
  ...
  for p in "${AI_PROVIDERS[@]}"; do
    ...
    [[ -n "${result}" && "${result}" != "LLM_ERROR" ]] && log_info "${p^^}: ${result}"
  done
}
...
launch_fastapi() {
  pip install fastapi uvicorn --quiet >/dev/null 2>&1 || true
...
import React from 'react'; import ReactDOM from 'react-dom/client'; const App = () => <div><h1>Enterprise OS v4.1</h1><button onClick={() => fetch('/ws')}>Run Audit</button></div>; ReactDOM.createRoot(document.getElementById('root')).render(<App />);
EOF
  cd "${GUI_DIR}"
...
EOF
  ...
  pip install celery redis --quiet >/dev/null 2>&1 || true
...
EOF
  pip install networkx --quiet >/dev/null 2>&1 || true
...
<!DOCTYPE html><html><head><title>Enterprise OS Docs</title></head><body><h1>Enterprise OS v4.1</h1><p>Autonomous Security Platform</p></body></html>
EOF
